...
class AwexSGLangAdapter(AwexInferenceAdapter):
    ...
    def _is_qwen3_5(self) -> bool:
        model_type = getattr(self._get_model().config, "model_type", "")
        return is_qwen3_5_model(model_type)

    def _get_vision_intermediate_size(self) -> int:
        return int(self._get_model().config.vision_config.intermediate_size)
    ...
    def _unfuse_qwen3_5_params(
        ...
        """Convert Qwen3.5 SGLang runtime weights to HuggingFace names/layouts."""
        ...
        if name.startswith("visual."):
            ...
            if "gate_up_proj" in name:
                ...
                if intermediate_size > projection_size:
                    raise ValueError(
                        "Vision intermediate size exceeds the SGLang projection "
                        f"size: intermediate_size={intermediate_size}, "
            ...
            if name.endswith("mlp.down_proj.weight"):
                ...
                if intermediate_size > tensor.shape[1]:
                    raise ValueError(
                        "Vision intermediate size exceeds the SGLang down "
                        f"projection input size: intermediate_size={intermediate_size}, "
